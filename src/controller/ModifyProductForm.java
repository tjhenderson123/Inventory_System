...
public class ModifyProductForm implements Initializable {
    ...
    private static Product passProduct;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        iDText.setText(String.valueOf(passProduct.getId()));
        nameText.setText(passProduct.getName());
        invText.setText(String.valueOf(passProduct.getStock()));
        priceCostText.setText(String.valueOf(passProduct.getPrice()));
        maxText.setText(String.valueOf(passProduct.getMax()));
        minText.setText(String.valueOf(passProduct.getMin()));

        for(Part p : passProduct.getAllAssociatedParts()){
    ...
    /**
     *
     * @param p sets passProduct to p
     */
    public static void receiveProduct(Product p){
        passProduct = p;
    }
    /**
     *
    ...
    /**
     *
     * @param actionEvent activated on remove associated part
     * @throws IOException indicates that method can throw an input/output exception (“How to Use the Throws....”, 2021)
     */
    public void onRemoveAssociatedPart(ActionEvent actionEvent) throws IOException {
        Part p1 = botTableView.getSelectionModel().getSelectedItem();
        if (p1 != null){
            ...
            if (result.isPresent() && result.get() == ButtonType.OK) {
                associatedParts.remove(p1);
            }
        }else {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setContentText("No data is present to delete.");
            alert.showAndWait();
        }
    ...
    /**
     *
     * @param actionEvent activate on save button
     * @throws IOException indicates that method can throw an input/output exception (“How to Use the Throws....”, 2021)
     */
    public void onSave(ActionEvent actionEvent) throws IOException {
        ...
        try {
            ...
            if (min <= max && stock >= min && stock <= max) {
                ...
                int indexOfProd = Inventory.getAllProducts().indexOf(passProduct);
                Product prod1 = new Product(id, name, price, stock, min, max);
                for (Part p : associatedParts) {
                    prod1.addAssociatedPart(p);
                }
                Inventory.updateProduct(indexOfProd, prod1);
    ...
    /**
     *
     * @param actionEvent activated when search text field is used
     */
